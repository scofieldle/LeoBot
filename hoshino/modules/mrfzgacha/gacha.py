...
class Gacha(object):
    ...
    def load_pool(self, pool_name:str):
        config = {}
        with open('/home/ubuntu/HoshinoBot/hoshino/modules/mrfzgacha/config.json') as f:
            config = json.load(f)
    ...
    def gacha_one(self, up_prob:int, s3_prob:int, s2_prob:int, s1_prob:int = None):
        ...
        if pick <= up_prob:
            return fromname(random.choice(self.up), 3), 100
        elif pick <= s3_prob:
            return fromname(random.choice(self.star3), 3), 50
        elif pick <= s2_prob + s3_prob:
            return fromname(random.choice(self.star2), 2), 10
        else:
            return fromname(random.choice(self.star1), 1), 1
