...
@cb_cmd(('出补时刀', '补刀', '补时'), ArgParser(usage='!出补时刀 <伤害值> (@qq)', arg_dict={
    '': ArgHolder(tip='伤害值', type=damage_int),
    '@': ArgHolder(tip='qq号', type=int, default=0),
    'R': ArgHolder(tip='周目数', type=round_code, default=0),
    'B': ArgHolder(tip='Boss编号', type=boss_code, default=0)}))
...
class SubscribeData:
    ...
    def __init__(self, data:dict):
        ...
        data.setdefault('lock', [])
        data.setdefault('shai?', False)
        data.setdefault('shai', [[]])
        self._data = data
    ...
    def get_sl_info(self):
        return self._data['sl']
    
    def set_lock(self, uid:int, ts):
        self._data['lock'].append([uid, ts])
    
    def set_sl(self, uid:int):
        self._data['sl'].append(uid)

    def clear_lock(self, lock_info):
        self._data['lock'].remove(lock_info)

    def clear_sl(self):
        self._data['sl'] = []

    def remove_sl(self, uid):
        self._data['sl'].remove(uid)

    def dump(self, filename):
        with open(filename, 'w', encoding='utf8') as f:
            json.dump(self._data, f, ensure_ascii=False)
        
    def set_shai_true(self):
        self._data['shai?'] = True

    def set_shai_false(self):
        self._data['shai?'] = False
        self._data['shai'] = []
    
    def get_shai_info(self):
        return self._data['shai']
    
    def remove_shai(self, shai_info):
        self._data['shai'].remove(shai_info)

    def get_shai(self):
        return self._data['shai?']

    def set_shai(self, uid, damage):
        self._data['shai'].append([uid,damage])
...
def sort_shai(shai_info):
    damage = []
    for i in shai_info:
        if 'w' in i:
            damage.append(int(i[1].split('w')[0]))
        else:
            damage.append(int(i[1][:2]))
    for i in range(len(damage) - 1):
        for j in (i+1,len(damage) - 1):
            if damage[i] > damage[j]:
                damage[i],damage[j] = damage[j],damage[i]
                shai_info[i],shai_info[j] = shai_info[j], shai_info[i]
    return shai_info
...
async def unlock_boss(bot:NoneBot, ctx:Context_T, args:ParseResult):
    ...
    sub = _load_sub(bm.group)
    lock = sub.get_lock_info()
    lock_number = len(lock)
    msg = f"正在出刀:"
    if lock:
        for i in range(lock_number):
            uid, ts = lock[i]
            mem = bm.get_member(uid, bm.group) or bm.get_member(uid, 0) or {'name': str(uid)}
            msg += f"\n{mem['name']}"
            if uid == ctx['user_id']:
                msg += f" 已取消出刀"
                sub.clear_lock(lock[i])
                _save_sub(sub, bm.group)
                lock_number = lock_number - 1
        if lock_number == len(lock):
            msg += f"\n当前出刀人数；{lock_number}"
            await bot.send(ctx, msg, at_sender=True)
        else:
            msg = "您尚未申请出刀！"
...
@cb_cmd(('调整状态'), ArgParser(usage='!调整状态 R<周目数> B<Boss号> <血量值>', arg_dict={
    'R': ArgHolder(tip='周目数', type=round_code),
    'B': ArgHolder(tip='Boss编号', type=boss_code),
    '': ArgHolder(tip='血量值', type=int)
}))
...
@cb_cmd('sl', ArgParser(usage='!sl (@qq)', arg_dict={
        '@': ArgHolder(tip='qq号', type=int, default=0)}))
async def send_sl(bot:NoneBot, ctx:Context_T, args:ParseResult):
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
    sub = _load_sub(bm.group)
    msg1 = f"已记录sl"
    msg2 = f"您已经sl过了！"
    sl_info = sub.get_sl_info()
    uid = args['@'] or args.at or ctx['user_id']
    if uid in sl_info:
        await bot.send(ctx, msg2, at_sender=True)
    else:
        sub.set_sl(uid)
        _save_sub(sub, bm.group)
        await bot.send(ctx, msg1, at_sender=True)
    
@cb_cmd('取消sl', ArgParser(usage='!取消sl (@qq)', arg_dict={
        '@': ArgHolder(tip='qq号', type=int, default=0)}))
async def send_sl(bot:NoneBot, ctx:Context_T, args:ParseResult):
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
...
@cb_cmd('清空sl', ArgParser(usage='!清空sl'))
async def delete_sl(bot:NoneBot, ctx:Context_T, args:ParseResult):
    _check_admin(ctx)
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
    sub = _load_sub(bm.group)
...
@cb_cmd('开始筛刀', ArgParser(usage='!开始筛刀'))
async def start_shai(bot:NoneBot, ctx:Context_T, args:ParseResult):
    _check_admin(ctx)
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
    sub = _load_sub(bm.group)
...
@cb_cmd('筛刀', ArgParser(usage='!筛刀 <伤害值> (@qq)', arg_dict={
    '': ArgHolder(tip='伤害值', type=str, default=''),
    '@': ArgHolder(tip='qq号', type=int, default=0)}))
async def send_shai(bot:NoneBot, ctx:Context_T, args:ParseResult):
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
    sub = _load_sub(bm.group)
    msg1 = f"已记录筛刀"
...
@cb_cmd('取消筛刀', ArgParser(usage='!取消筛刀'))
async def start_shai(bot:NoneBot, ctx:Context_T, args:ParseResult):
    uid = ctx['user_id']
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
...
@cb_cmd('筛刀记录', ArgParser(usage='!筛刀记录'))
async def start_shai(bot:NoneBot, ctx:Context_T, args:ParseResult):
    _check_admin(ctx)
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
    ...
    if sub.get_shai():
        shai_info = sub.get_shai_info()
        msg += str(len(shai_info))
        shai_info = sort_shai(shai_info)
        for i in range(len(shai_info)):
            uid, damage = shai_info[i]
            mem = bm.get_member(uid, bm.group) or bm.get_member(uid, 0) or {'name': str(uid)}
            msg += f"\n{mem['name']}"
            msg += ', '
            msg += damage
...
@cb_cmd('结束筛刀', ArgParser(usage='!结束筛刀'))
async def start_shai(bot:NoneBot, ctx:Context_T, args:ParseResult):
    _check_admin(ctx)
    bm = BattleMaster(ctx['group_id'])
    _check_clan(bm)
