...
class Roster:
    ...
    def update(self):
        importlib.reload(_kgqgj_data)
        self._roster.clear()
        for idx, names in _kgqgj_data.CHARA_NAME.items():
            for n in names:
...
class Chara:
    ...
    @property
    def name(self):
        return _kgqgj_data.CHARA_NAME[self.id][0] if self.id in _kgqgj_data.CHARA_NAME else _kgqgj_data.CHARA_NAME[UNKNOWN][0]
