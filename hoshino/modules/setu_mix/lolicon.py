...
def load_native_info(sub_dir):
    info = {}
    res = './res/img/setu_mix/' + sub_dir
    if not os.path.exists(res):
        return info
    fnlist =  os.listdir(res)
    ...
    for fn in fnlist:
        ...
        uid = int(s[0])
        try:
            with open(res + '/' + fn, encoding='utf8') as f:
                d = json.load(f)
...
def save_image(image):
    res = f'./res/img/setu_mix/lolicon/{image["id"]}'
    if image['r18']:
        res = f'./res/img/setu_mix/lolicon_r18/{image["id"]}'
...
def get_setu_native(r18 = 0, uid = 0):
    ...
    path = f'./res/img/setu_mix/lolicon'
    if r18 == 1:
        path = f'./res/img/setu_mix/lolicon_r18'
    elif r18 == 2:
    ...
    try:
        with open(path + '.jpg', 'rb') as f:
            image['data'] = f.read()
        with open(path + '.json', encoding='utf8') as f:
            d = json.load(f)
            if 'title' in d:
                image['title'] = d['title']
            if 'author' in d:
