...
def load_native_info(sub_dir):
    ...
    for fn in fnlist:
        ...
        uid = int(s[0])
        try:
            with open(path + '/' + fn, encoding='utf8') as f:
                d = json.load(f)
...
def save_image(image, mode='acggov'):
    path = f'./res/img/setu_mix/{mode}/{image["id"]}'
...
async def get_setu_online():
    ...
    #检查本地是否存在该图片
    path = f'./res/img/setu_mix/acggov/{image["id"]}.jpg'
...
def get_setu_native(uid = 0):
    ...
    path = f'./res/img/setu_mix/acggov'
    if not os.path.exists(path):
        return image
    ...
    path = f'./res/img/setu_mix/acggov/{uid}'
    try:
